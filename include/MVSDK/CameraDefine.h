...
/// \~english Image Sensor Operation Mode in RSCamera
typedef enum 
{
    /// \~chinese 连续采集模式
...
参数文件存在安装目录的 \RSCamera\Configs 目录下，以config为后缀名的文件。
*/
typedef enum
{
...
/// \note			0：上次掉线原因，网络通讯失败
/// \note			1：上次掉线原因，相机掉电
/// \~english RSCamera connection status callback
/// \param [in] hCamera RSCamera handle
/// \param [in] MSG message, 0: RSCamera disconnected 1: RSCamera connection restored
